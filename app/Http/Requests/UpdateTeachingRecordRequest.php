...
class UpdateTeachingRecordRequest extends FormRequest
{
    /**
    ...
    {
        return $this->route('teaching_record')->user_id === auth()->id();
    }
    ...
    public function withValidator($validator)
    {
        $validator->after(function ($validator) {
            if ($this->filled('teaching_date') && $this->filled('start_time') && $this->filled('end_time')) {
                // Calculate duration for this updated record
                ...
                // Get existing records for this user and date, excluding current record
                $existingRecords = TeachingRecord::forUser(auth()->id())
                    ->forDate($this->teaching_date)
                    ->where('id', '!=', $this->route('teaching_record')->id)
                    ->get();
                ...
                if ($totalMinutes > 600) {
                    ...
                    $validator->errors()->add(
                        'end_time', 
                        "Updating this session would exceed the 10-hour daily limit. You have {$remainingTime} remaining for this date."
                    );
                }
            }
        });
